...
resource "aws_vpc" "cognito-lambda-vpc" {
  ...
  tags = {
    Name = "cognito-lambda-vpc"
  }
}

resource "aws_subnet" "cognito-lambda-subnet" {
  cidr_block = "10.0.1.0/24"
  vpc_id = aws_vpc.cognito-lambda-vpc.id
  availability_zone = "us-east-1a"

  tags = {
    Name = "cognito-lambda-subnet"
  }
}

resource "aws_security_group" "cognito-lambda-sg" {
  name_prefix = "cognito-lambda-sg"
  vpc_id = aws_vpc.cognito-lambda-vpc.id
...
resource "aws_iam_role" "cognito-lambda-role" {
...
resource "aws_iam_role_policy_attachment" "cognito-lambda-policy-attachment" {
  policy_arn = aws_iam_policy.cognito-lambda-policy.arn
  role = aws_iam_role.cognito-lambda-role.name
}
...
resource "aws_lambda_function" "cognito-define-auth-challenge-lambda" {
  ...
  vpc_config {
    ...
    security_group_ids = [aws_security_group.cognito-lambda-sg.id]
  }
}
...
resource "aws_lambda_permission" "allow_execution_from_user_pool_1" {
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.cognito-define-auth-challenge-lambda.function_name
